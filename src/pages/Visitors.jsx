...
function Visitors() {
  const [visitantes, setVisitantes] = useState([
    { name: "Ana", doc: "RG 45.678.123-0", vehicle: "ABC-1234", entry: false },
    { name: "Bruno", doc: "CNH 987654321", vehicle: "XYZ-9876", purpose: "Entrega", entry: true, entryTime: "09:15" },
    { name: "Clara", doc: "RG 33.555.111-9", vehicle: "KLM-4532", entry: false },
  ])
      ...
      setVisitantes(prev =>
        prev.map(v =>
          v.name === selectedVisitor.name
            ? { ...v, entry: false, entryTime: null, purpose: null }
            : v
        )
      )
    } else if (actionType === "delete") {
      setVisitantes(prev => prev.filter(v => v.name !== selectedVisitor.name))
    }
  ...
  const handleReasonConfirm = (title, description) => {
    const horaAtual = new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })

    setVisitantes(prev =>
      prev.map(v =>
        v.name === selectedVisitor.name
          ? { ...v, entry: true, purpose: title, description, entryTime: horaAtual }
          : v
      )
    )
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <main className="flex-1 p-6 transition-all duration-300">
        ...
        <div className="flex justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-800 mb-4">Visitantes</h1>
            <p className="text-gray-600 mb-4">Controle de entrada e saída de visitantes</p>
          </div>
          <Link
        ...
        <div className="m-5 w-[1000px] rounded-md">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mt-8 mb-2">Visitantes Presentes</h2>
          ...
          {visitantesPresentes.length > 0 ? (
            visitantesPresentes.map((v, index) => (
              <div
                ...
                className="group grid grid-cols-5 items-center p-3 bg-white rounded-md transform transition-all duration-300 hover:bg-blue-600 hover:text-white hover:scale-[1.02] hover:shadow-lg"
              >
                ...
                <div className="flex items-center gap-2">
                  <CheckCircle size={20} className="text-green-500 group-hover:text-white transition-all duration-300" />
                  <span>Presente</span>
                </div>
          ...
          {visitantesAusentes.length > 0 ? (
            visitantesAusentes.map((v, index) => (
              ...
              >
                ...
                <p>{v.vehicle}</p>
                <div className="flex items-center gap-2">
                  <XCircle size={20} className="text-red-500 group-hover:text-white transition-all duration-300" />
                  <span>Ausente</span>
                </div>
                <div className="flex justify-center gap-3">
                  <button
            ...
            <p className="text-gray-500 p-3">Nenhum visitante aguardando entrada.</p>
          )}
        </div>
      </main>
